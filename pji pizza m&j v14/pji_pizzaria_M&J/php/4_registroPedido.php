...
if (isset(
    ...
    for ($i = 0; $i < count($qtdProdutos); $i++) {
        ...
        if ($quantidade > 0) {
             //select pra pegar  valor das pizzas, e calcular o subTotal.
             $sqlPreco = "SELECT valorProduto from Produtos WHERE idProduto = $i";
             $result = $conexao->query($sqlPreco);
            ...
            $sqlPedidoProduto = "INSERT INTO PedidoProduto (quantidade, pedido_fk, produto_fk, subTotal) VALUES (?, ?, ?, ?)";

            if ($stmt = mysqli_prepare($conexao, $sqlPedidoProduto)) {

                mysqli_stmt_bind_param($stmt, 'dddd', $quantidade, $idPedido_fk, $idProduto, $subTotal);
